...
function Flag({ code, size = 28 }) {
  if (!code)
    return (
      <span
        style={{ width: size, height: size * 0.75, display: 'inline-block' }}
      />
    );
  return (
    <img
...
export default function Matches() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      ms.forEach((m) => {
        ...
        init[m.id] = {
          home: p ? String(p.home_pred) : '',
          away: p ? String(p.away_pred) : '',
          dirty: false,
          hasSaved: !!p,
          category: p?.category,
          points: p?.points,
  ...
  function setField(id, field, value) {
    setInputs((p) => ({
      ...p,
      [id]: { ...p[id], [field]: value, dirty: true },
    }));
  ...
  async function saveOne(matchId) {
    ...
    try {
      ...
      setInputs((p) => ({
        ...p,
        [matchId]: { ...p[matchId], dirty: false, hasSaved: true },
      }));
    } catch {
    } finally {
  ...
  async function saveAll() {
    await Promise.all(pendingIds.map(saveOne));
  }

  if (loading)
    return (
      <div className='text-center py-5 text-muted'>
        <span className='spinner-border me-2'></span>Cargando partidos...
      </div>
    );
              ...
              return (
                <div key={m.id} className='match-card'>
                  ...
                  >
                    <span>
                      📅{' '}
                      {new Date(m.kickoff).toLocaleDateString('es-AR', {
                        weekday: 'long',
                        day: 'numeric',
                        month: 'long',
                      })}
                  ...
                  >
                    <div className='d-flex align-items-center gap-2'>
                      {isOk ? (
                        ...
                        <span
                          className={`badge border ${CAT_CLASS[inp.category]}`}
                    ...
                    {inp?.hasSaved && (
                      <div className='d-flex align-items-center gap-2'>
                        ...
                        {inp.category && (
                          ...
                          >
                            {inp.category}{' '}
                            {inp.points != null ? `· ${inp.points}pts` : ''}
                          </span>
                        )}
                      </div>
                    )}
      ...
      {pendingIds.length > 0 && (
        <div className='save-bar'>
          <span className='text-muted' style={{ fontSize: '0.9rem' }}>
            ...
            {pendingIds.length} pronóstico{pendingIds.length > 1 ? 's' : ''} sin
            guardar
          </span>
          <button onClick={saveAll} className='btn btn-primary fw-bold px-4'>
